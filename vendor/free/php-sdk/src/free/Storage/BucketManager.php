...
final class BucketManager
{
    ...
    {
        $path = '/stat/' . \free\entry($bucket, $key);
    ...
    {
        $path = '/delete/' . \free\entry($bucket, $key);
    ...
    {
        $from = \free\entry($from_bucket, $from_key);
        $to = \free\entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        list($_, $error) = $this->rsPost($path);
        return $error;
    }
    ...
    {
        $resource = \free\entry($bucket, $key);
        $encode_mime = \free\base64_urlSafeEncode($mime);
    ...
    public function fetch($url, $bucket, $key)
    {
        ...
        $resource = \free\base64_urlSafeEncode($url);
        $to = \free\entry($bucket, $key);
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . \free\entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = \free\entry($source_bucket, $from_key);
            $to = \free\entry($target_bucket, $to_key);
